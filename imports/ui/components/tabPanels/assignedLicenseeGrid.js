import './assignedLicenseeGrid.html';
...
Template.assignedLicenseeGrid.onRendered(function renderLicenseeGrid() {
    this.autorun(() => {
        let data = [];


        if (Meteor.users.find({}).count() > 0) {
            console.log('id', Session.get('activeAdminId'));
            data = Meteor.users.find({
                $and: [
                    { adminRole: false },
                    { suRole: false },
                    { managedByAdmins: { $in: [Session.get('activeAdminId')] } }
                ],
                isDeleted: false
            }).fetch().map((el) => {
                var user = [];
        ...
        $('#jsGrid_assignedLicensee').jsGrid({
            height: 'auto',
            width: '100%',
                    ...
                    itemTemplate(value, data) {
                        return $('<button>')
                            ...
                            .on('click', (e) => {
                                ...
                                Meteor.call('users.unAssign.licensee', activeAdminId, data.userId);
                                e.stopPropagation();
                            });
                    },
                },
